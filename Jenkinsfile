    ...
    stages {
        ...
        stage('Deploy cf-blog on Kubernetes cluster') {
            steps {
                ...
                sh 'ssh k8s-master -l gsmcfdevops -i ~/.ssh/gsmcfdevops -o StrictHostKeyChecking=no "kubectl delete secret $APP_NAME-postgresqlpassword -n $APP_NAME || true && kubectl create secret generic $APP_NAME-postgresqlpassword --from-literal=APP_DB_PASSWORD=$APP_PROD_DB_PASSWORD -n $APP_NAME"'
                sh 'ssh k8s-master -l gsmcfdevops -i ~/.ssh/gsmcfdevops -o StrictHostKeyChecking=no "kubectl delete secret $APP_NAME-postgresqldatabase -n $APP_NAME || true && kubectl create secret generic $APP_NAME-postgresqldatabase --from-literal=APP_DB_NAME=$APP_PROD_DB_NAME -n $APP_NAME"'
